...
class conv(operator):
    ...
    def forward(self, input, weights, bias):
        ...
        output1 = np.matmul(weights.reshape(out_channel, -1), input_conv) + bias.reshape(out_channel, -1)
        output = output1.reshape(batch, out_channel, out_height, out_width)
        #####################################################################################
        return output
...
class gru(operator):
    ...
    def forward(self, input, kernel, recurrent_kernel):
        ...
        # new gate
        x_h = np.tanh((x_r * prev_h).dot(recurrent_kernel_h) + x.dot(kernel_h))
        #####################################################################################
    ...
    def backward(self, out_grad, input, kernel, recurrent_kernel):
        ...
        x_z_grad = out_grad * (-x_h + prev_h) # x_h:batch,units | prev_h:batch,units | out_grad:batch,units | x_z_grad:batch,units
        x_h_grad = out_grad * (1 - x_z) # x_z:batch,units | out_grad:batch,units | x_h_grad:batch,units

        tanh_h_grad = x_h_grad * (1 - np.square(x_h)) # x_h_grad:batch,units | x_h:batch,units | tanh_h_grad:batch,units
        x_r_grad = tanh_h_grad.dot(recurrent_kernel_h.T) * prev_h # tanh_h_grad:batch,units | recurent_kernel_z:units, units | prev_h:batch,units | x_r_grad:batch,units

        sig_r_grad = x_r_grad * x_r * (1 - x_r) # x_r_grad:batch,units | x_r:batch,units | sig_r_grad:batch,units
        sig_z_grad = x_z_grad * x_z * (1 - x_z) # x_z_grad:batch,units | x_z:batch,units | sig_z_grad:batch,units

        # x_grad_out, prev_h_grad_out = out_grad
        x_grad = sig_r_grad.dot(kernel_r.T) + sig_z_grad.dot(kernel_z.T) + tanh_h_grad.dot(kernel_h.T) # sig_r_grad:batch,units | kernel_r:feature, units |sig_z_grad:batch,units | kernel_z:feature, units | tanh_h_grad:batch,units | kernel_h:feature, units | x_grad:batch,features
        prev_h_grad = sig_r_grad.dot(recurrent_kernel_r.T) + sig_z_grad.dot(recurrent_kernel_z.T) + (tanh_h_grad.dot(recurrent_kernel_h.T) * x_r) + out_grad* x_z # sig_r_grad:batch,units | recurrent_kernel_r:units, units | prev_h_grad: batch,units

        kernel_r_grad = ((sig_r_grad.T).dot(x)).T  # sig_r_grad:batch,units | x:batch,features | kernel_r_grad: features,units
        kernel_z_grad = ((sig_z_grad.T).dot(x)).T  # sig_z_grad:batch,units | x:batch,features | kernel_z_grad: features,units
        kernel_h_grad = ((tanh_h_grad.T).dot(x)).T # tanh_h_grad:batch,units | x:batch,features | kernel_h_grad: features,units

        recurrent_kernel_r_grad = ((sig_r_grad.T).dot(prev_h)).T # sig_r_grad:batch,units | prev_h:batch,units | recurrent_kernel_r_grad:units,units
        recurrent_kernel_z_grad = ((sig_z_grad.T).dot(prev_h)).T # sig_z_grad:batch,units | prev_h:batch,units | recurrent_kernel_z_grad:units,units
        recurrent_kernel_h_grad = ((tanh_h_grad.T).dot(x_r * prev_h)).T    #x_h_grad * (1 - np.square(x_h)) * x_r * prev_h
        #####################################################################################
