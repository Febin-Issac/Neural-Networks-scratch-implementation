...
class BiRNN(Layer):
    ...
    def forward(self, input):
        ...
        #####################################################################################
        # code here
        output = self.forward_rnn.forward(input)
        # output = self._reverse_temporal_data(output)

        output = np.concatenate((output, self._reverse_temporal_data(self.backward_rnn.forward(self._reverse_temporal_data(input, mask)), mask)), axis=2)
        # output = None
        #####################################################################################
...
class BiGRU(Layer):
    ...
    def __init__(self, in_features, units, h0=None, hr=None, name='biGRU', initializer=Gaussian()):
        """Initialize two inner RNNs for forward and backward processes, respectively
        ...
        # Arguments
            ...
            hr: default initial state for backward phase, numpy array with shape (units,)
        """
        super(BiGRU, self).__init__(name=name)
        self.trainable = True
        self.forward_rnn = GRU(in_features, units, h0,
                                      'forward_rnn', initializer=initializer)
        self.backward_rnn = GRU(in_features, units, hr,
                                       'backward_rnn', initializer=initializer)
    ...
    def forward(self, input):
        ...
        output = np.concatenate((output, self._reverse_temporal_data(
            self.backward_rnn.forward(self._reverse_temporal_data(input, mask)), mask)), axis=2)
        # output = None
        #####################################################################################
